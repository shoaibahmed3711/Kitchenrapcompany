...
const ManageCurrency = () => {
  ...
  const filteredRows = rows.filter((row) => {
    const name = row.name ? row.name.toLowerCase() : ""; // Change variable to name
  ...
  return (
    ...
    <div className='flex flex-row m-[1vw] gap-[1vw] items-center image-hover-effect'>
      <div className='w-[3vw]'>
      <img src="/Account/currency.png" className="image-hover-effect" alt="Leave" />
      </div>
      <h1 className=' text-[2vw] text-[#E9278E]'>Manage Current</h1>
      </div>
        ...
        <table className="w-[80vw] overflow-y-auto">
          <thead className="bg-gray-300 w-[80vw]">
            <tr className="w-[80vw]">
              ...
              <th className="border p-[0.5vw] text-[1vw]">Effective Date</th>
              <th className="border p-[0.5vw] text-[1vw]">Source Currency</th>
              <th className="border p-[0.5vw] text-[1vw]">Rate</th>
              <th className="border p-[0.5vw] text-[1vw]">Target Currency</th>
              <th className="border p-[0.5vw] text-[1vw]">Set By</th>
          ...
          <tbody className="rounded-lg bg-gray-100 w-[80vw] text-center">
            {filteredRows.map((row, index) => (
              <tr key={index}>
                ...
                <td>{row.SourceCurrency}</td> {/* Show Source Currency */}
                <td>{row.Rate}</td> {/* Show Rate */}
                <td>{row.TargetCurrency}</td> {/* Show Target Currency */}
                <td>{row.SetBy}</td> {/* Show Set By */}

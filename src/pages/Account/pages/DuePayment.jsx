...
const DuePayment = () => {
  ...
  const handleExport = () => {
    ...
    rows.forEach((row, index) => {
      ...
      // Assuming EntryDate, EntryDescription, Type, Approval, Amount, TotalPaidAmount, DuePayment, Status are keys in 'row'
      doc.text(
        `${row.EntryDate} - ${row.EntryDescription} - ${row.Type} - ${row.Approval} - ${row.Amount} - ${row.TotalPaidAmount} - ${row.DuePayment} - ${row.Status}`,
        10,
        yPos
      );
    });
  ...
  const filteredRows = rows.filter((row) => {
    ...
    if (filter === 'All') {
      ...
      return (
        status === filter.toLowerCase() && BranchName.includes(searchTerm.toLowerCase())
      );
    }
  });
  ...
  return (
    ...
    <div className='flex flex-row m-[1vw] gap-[1vw] items-center image-hover-effect'>
      <div className='w-[3vw]'>
      <img src="/Account/due.png" className="image-hover-effect" alt="Leave" />
      </div>
      <h1 className=' text-[2vw] text-[#E9278E]'>Due Payment</h1>
      </div>
        ...
        <table className="w-[80vw] overflow-y-auto">
          <thead className="bg-gray-300 w-[80vw]">
            <tr className="w-[80vw]">
              ...
              <th className="border p-[0.5vw] text-[1vw]">Entry Description</th>
              <th className="border p-[0.5vw] text-[1vw]">Type</th>
              <th className="border p-[0.5vw] text-[1vw]">Approval Date & Time</th>
              <th className="border p-[0.5vw] text-[1vw]">Amount</th>
              <th className="border p-[0.5vw] text-[1vw]">Total Paid Amount</th>
              <th className="border p-[0.5vw] text-[1vw]">Due Payment</th>

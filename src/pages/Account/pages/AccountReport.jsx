...
const AccountReport = () => {
  ...
  const filteredRows = rows.filter((row) => {
    const name = row.EntryDate ? row.EntryDate.toLowerCase() : '';
    const status = row.status ? row.status.toLowerCase() : '';

    if (filter === 'All') {
      return name.includes(searchTerm.toLowerCase());
    } else {
      return status === filter.toLowerCase() && name.includes(searchTerm.toLowerCase());
    }
  });
  ...
  return (
    ...
    <div className='flex flex-row m-[1vw] gap-[1vw] items-center image-hover-effect'>
      <div className='w-[3vw]'>
      <img src="/Account/account report.png" className="image-hover-effect" alt="Leave" />
      </div>
      <h1 className=' text-[2vw] text-[#E9278E]'>Account Report</h1>
      </div>
        ...
        <table className="w-[80vw] overflow-y-auto">
          <thead className="bg-gray-300 w-[80vw]">
            <tr className="w-[80vw]">
              ...
              <th className="border p-[0.5vw] text-[1vw]">Account Code</th>
              <th className="border p-[0.5vw] text-[1vw]">Account Type</th>

...
const InvoiceTypes = () => {
  ...
  const handleExport = () => {
    ...
    rows.forEach((row, index) => {
      ...
      doc.text(`${row.name} - ${row.direction}`, 10, yPos);
    });
  ...
  const filteredRows = rows.filter((row) => {
    const BranchName = row.name ? row.name.toLowerCase() : '';
    const status = row.status ? row.status.toLowerCase() : '';

    if (filter === 'All') {
      return BranchName.includes(searchTerm.toLowerCase());
    } else {
      return status === filter.toLowerCase() && BranchName.includes(searchTerm.toLowerCase());
    }
  });
  ...
  return (
    ...
    <div className='flex flex-row m-[1vw] gap-[1vw] items-center image-hover-effect'>
      <div className='w-[3vw]'>
      <img src="/invoice/types.png" className="image-hover-effect" alt="Leave" />
      </div>
      <h1 className=' text-[2vw] text-[#E9278E]'>Invoice Types</h1>
      </div>
